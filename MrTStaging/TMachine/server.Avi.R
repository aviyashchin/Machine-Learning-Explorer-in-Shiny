...
shinyServer(function(input,output,session)
{
  ...
  if(RUNSTANDALONE){
    rawData <- "text/csv" # input$rawInputFile 
    #accept=c('text/csv', 'text/comma-separated-values,text/plain', '.csv')),
    headerTag <- TRUE #input$headerUI
    sepTag <-  "," #input$sepUI
    #c(Comma=',',Semicolon=';',Tab='\t'),'Comma')
    quoteTag <- '"' #input$quoteUI 
    #c(None='','Double Quote'='"','Single Quote'="'"),'Double Quote')
    rawData$datapath <-"/Users/avi/programming/boxer/MrTStaging/TMachine/iris.csv"
...
#     input$runAnalysisUI;
    ...
    ########################################################
    if(RUNSTANDALONE){
      column = "Sepal.Length";#colnames(data)[1] #isolate(input$modelLabelUI);  #WHERE IS THIS USED? 

      columnElement = which(colnames(newData) == column);
      foldsType = 'cv' #input$crossFoldTypeUI=        # c("K-Fold CV"='cv',"Repeated KFold CV"="repeatedcv"),"K-Fold CV")
      folds =  5 # input$foldsUI
      numberOfRepeats =  5 #input$repeatUI
      preprocessType =  "" #input$preprocessingUI      #c('No Preprocessing'="",'PCA'="pca",'ICA'="ica"),'No Preprocessing')
      kFolds = 5 #isolate(input$foldsUI);
    ...
    if(RUNSTANDALONE){
      ...
      if(RUNSTANDALONE){
        alphaStart = 0.1 #input$enAlphaStartUI 
        alphaEnd = 1.0 #input$enAlphaEndUI 
        alphaRange = 5 #input$enAlphaRangeUI
        lambdaStart = 0.1 #input$enLambdaStartUI
        lambdaEnd = 1 #input$enLambdaEndUI
        lambdaRange = 5 #input$enLambdaRangeUI
        familyData = "Binomial" #input$enModelTypeUI
...
#        results = train(form,data=newData,tuneGrid=gridding,method="glmnet",family=familyData,trControl=control,preProcess=preprocessType);
      ...
      if(RUNSTANDALONE){
        mTryStartEnd <- c(1,4) # = isolate(input$mTryRangeUI)
        nMtry <- 1 # = isolate(input$rmTryUI);        
        familyData <- "Binomial" # = isolate(input$rfModelTypeUI);c("Binomial","Gaussian","Multinomial"),"Binomial"),
        preprocessType =  'pca' #input$preprocessingUI      #c('No Preprocessing'="",'PCA'="pca",'ICA'="ica"),'No Preprocessing')
      ...
      if(RUNSTANDALONE){
        familyData <- 5 #= isolate(input$nnModelTypeUI);  c('Binomial'="binomial",'Gaussian'="gaussian",'Multinomial'="multinomial"),"Binomial"),
        nnRange <- c(1,5) #= isolate(input$nnSizeUI);
        numNN <- 5 #= isolate(input$nnSizeRangeUI);
        nnDecayRange <- c(0,0.1) #= isolate(input$nnDecayUI);
        numnnDecayRange <- 5 #= isolate(input$nnDecayRangeUI);
