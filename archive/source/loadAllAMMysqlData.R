loadAllAMMysqlData <- function(){
  ...
  NumCompanies <- runSQL(con,"select `FIPS`,`Number of establishments`,  `Annual payroll ($1,000)` as `annual_payroll` from `county business patterns from factfinder.census.gov1` where year=2013 and `Meaning of 2012 NAICS code`='Total for all sectors';")
  #`Paid employees for pay period including March 12 (number)` as `num_paid_employees`,`First-quarter payroll ($1,000)` as `first_q_payroll`,
